...
import { listSavedConfigurations, saveQuote, saveQuoteSchema } from "@/application/ecp/platform-service";
...
export async function GET() {
  ...
  if (!user) {
    return NextResponse.json({ message: "Authentication required.", loginUrl: "/login?returnTo=%2Fsaved-configurations" }, { status: 401 });
  }

  return NextResponse.json(await listSavedConfigurations(user.id, prismaSavedQuoteRepository));
}
